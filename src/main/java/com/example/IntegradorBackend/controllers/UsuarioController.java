...
public class UsuarioController {
    ...
    public ResponseEntity<?> modificar(@RequestBody Usuario usuario, @PathVariable Long id) {
        ...
        if (getId.isPresent()) {
            ...
            throw new RequestException("400 Bad Request","No existe ningún usuario para el id "+id);
        }
    }
    ...
    public ResponseEntity<?> buscarEmail(@PathVariable String email){
        ...
        if (getCorreo == null){
            throw new RequestException("400 Bad Request","No existe ningún usuario con email "+ email);
        } else {
